...
export function useInventory(params: { search?: string; categoryId?: string; includeLowStock?: boolean } = {}) {
  return useQuery({ queryKey: ['inventory', params], queryFn: () => inventoryApi.getInventory(params).then(r => r.data) })
}

export function useProductInventory(productId: string) {
  return useQuery({ queryKey: ['inventory', productId], queryFn: () => inventoryApi.getProductInventory(productId).then(r => r.data), enabled: !!productId })
}

export function useInventoryAlerts() {
  return useQuery({ queryKey: ['inventory-alerts'], queryFn: () => inventoryApi.getAlerts().then(r => r.data) })
}
...
export function useInventoryMovements(params: { productId?: string; from?: string; to?: string; page?: number; size?: number } = {}) {
  return useQuery({ queryKey: ['inventory-movements', params], queryFn: () => inventoryApi.getMovements(params).then(r => r.data) })
}

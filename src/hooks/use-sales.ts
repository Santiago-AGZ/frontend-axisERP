...
export function useCustomers(params: { search?: string; documentNumber?: string; phone?: string } = {}) {
  return useQuery({ queryKey: ['customers', params], queryFn: () => salesApi.listCustomers(params).then(r => r.data) })
}
...
export function useSales(params: { from?: string; to?: string; customerId?: string; invoiceNumber?: string; paymentStatus?: string; page?: number; size?: number } = {}) {
  return useQuery({ queryKey: ['sales', params], queryFn: () => salesApi.listSales(params).then(r => r.data) })
}

export function useSale(id: string) {
  return useQuery({ queryKey: ['sales', id], queryFn: () => salesApi.getSale(id).then(r => r.data), enabled: !!id })
}
